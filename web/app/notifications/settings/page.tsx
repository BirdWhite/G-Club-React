...
import { Bell, Gamepad2, Users, Lightbulb, Moon, Clock, PenTool, Megaphone, Settings, Calendar } from 'lucide-react';
    ...
    <div className="bg-background">
      <div className="flex flex-col items-center px-8 sm:px-10 lg:px-12 py-8">
        <div className="w-full max-w-4xl">
          ...
          {/* 마스터 알림 토글 */}
          <div className="mb-6 sm:mb-8">
            <div className={`p-4 sm:p-6 rounded-2xl shadow-lg border border-border transition-colors ${isMasterEnabled
            ...
            <div className="mb-6 p-4 sm:p-6 bg-card rounded-2xl shadow-lg border border-border">
              ...
              <div className="space-y-3">
                {allDeviceSubscriptions.map((device, index) => {
                  ...
                  return (
                    <div key={index} className={`p-3 rounded-lg border ${isCurrentDevice
          ...
          <div className="space-y-4">
            {/* 방해 금지 시간 설정 */}
            <div className={`p-4 sm:p-6 rounded-2xl shadow-lg border border-border transition-colors min-h-[5rem] sm:min-h-[6rem] flex items-center ${settings.doNotDisturb.enabled
                ? 'bg-card hover:bg-card/80'
                : 'bg-card hover:bg-card/80'
              }`}>
              <div className="flex items-center justify-between w-full gap-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {['일', '월', '화', '수', '목', '금', '토'].map((day, index) => (
                      <button
                        ...
                        className={`px-3 py-2 rounded-md text-sm font-medium transition-colors ${settings.doNotDisturb.days.includes(index.toString())
            ...
            {/* 신규 게임메이트 글 알림 */}
            <div className={`p-4 sm:p-6 rounded-2xl shadow-lg border border-border transition-colors min-h-[5rem] sm:min-h-[6rem] flex items-center ${settings.newGamePost.enabled
                ? 'bg-card hover:bg-card/80'
                : 'bg-card hover:bg-card/80'
              }`}>
              <div className="flex items-center justify-between w-full gap-3">
            ...
            {/* 참여중인 모임 알림 */}
            <div className={`p-4 sm:p-6 rounded-2xl shadow-lg border border-border transition-colors min-h-[5rem] sm:min-h-[6rem] flex items-center ${settings.participatingGame.enabled
                ? 'bg-card hover:bg-card/80'
                : 'bg-card hover:bg-card/80'
              }`}>
              <div className="flex items-center justify-between w-full gap-3">
            ...
            {/* 내가 작성한 모임 알림 */}
            <div className={`p-4 sm:p-6 rounded-2xl shadow-lg border border-border transition-colors min-h-[5rem] sm:min-h-[6rem] flex items-center ${settings.myGamePost.enabled
                ? 'bg-card hover:bg-card/80'
                : 'bg-card hover:bg-card/80'
              }`}>
              <div className="flex items-center justify-between w-full gap-3">
            ...
            {/* 공지사항 알림 */}
            <div className={`p-4 sm:p-6 rounded-2xl shadow-lg border border-border transition-colors min-h-[5rem] sm:min-h-[6rem] flex items-center ${settings.notice?.enabled
                ? 'bg-card hover:bg-card/80'
                : 'bg-card hover:bg-card/80'
              }`}>
              <div className="flex items-center justify-between w-full gap-3">
            ...
            {/* 예비 참여 알림 */}
            <div className={`p-4 sm:p-6 rounded-2xl shadow-lg border border-border transition-colors min-h-[5rem] sm:min-h-[6rem] flex items-center ${settings.waitingList.enabled
                ? 'bg-card hover:bg-card/80'
                : 'bg-card hover:bg-card/80'
              }`}>
              <div className="flex items-center justify-between w-full gap-3">
            ...
            {/* 일정 알림 */}
            <div className={`p-4 sm:p-6 rounded-2xl shadow-lg border border-border transition-colors min-h-[5rem] sm:min-h-[6rem] flex items-center ${settings.calendarEvent?.enabled
                ? 'bg-card hover:bg-card/80'
                : 'bg-card hover:bg-card/80'
              }`}>
              <div className="flex items-center justify-between w-full gap-3">
                <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="font-semibold text-sm sm:text-base text-foreground">일정 알림</h3>
                    <p className="text-xs sm:text-sm text-muted-foreground">새로운 일정 등록 및 리마인더 알림</p>
                  </div>
                </div>
                <div className="flex items-center gap-2 sm:gap-3 flex-shrink-0">
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input
                      type="checkbox"
                      checked={settings.calendarEvent?.enabled ?? true}
                      onChange={() => toggleCategory('calendarEvent')}
                      className="sr-only peer"
                    />
                    <div className="w-11 h-6 bg-card-muted peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-primary/30 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-card-muted after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-primary"></div>
                  </label>
