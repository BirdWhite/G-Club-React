...
export function formatRelativeTime(date: Date | string): string {
  ...
  if (isToday(targetDate)) {
    return `오늘 ${format(targetDate, 'HH:mm', { locale: ko })}`;
  } else if (isYesterday(targetDate)) {
    return `어제 ${format(targetDate, 'HH:mm', { locale: ko })}`;
  } else if (isTomorrow(targetDate)) {
    return `내일 ${format(targetDate, 'HH:mm', { locale: ko })}`;
  } else if (isThisYear(targetDate)) {
    return format(targetDate, 'M월 d일 (E) HH:mm', { locale: ko });
  } else {
    return format(targetDate, 'yyyy년 M월 d일 (E) HH:mm', { locale: ko });
  }
}
