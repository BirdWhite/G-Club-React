...
export function startGamePostCleanup() {
  ...
  cron.schedule('0,30 * * * *', async () => {
    ...
    try {
      ...
      const { data: fullImmediatePosts, error: fullImmediateError } = await supabase
        .from('GamePost')
        .select('id, title, startTime, status, isFull')
        .eq('status', 'OPEN')
        .eq('isFull', true)
      ...
      if (fullImmediateError) {
        throw fullImmediateError;
      }

      const allFullPosts = [...(fullPosts || []), ...(fullImmediatePosts || [])];

      if (allFullPosts.length > 0) {
        console.log(`게임 시작할 FULL 상태 글 ${allFullPosts.length}개 발견:`);
        allFullPosts.forEach(post => {
          console.log(`- ID: ${post.id}, 제목: ${post.title}, 시작시간: ${post.startTime || '바로출발'}`);
        });

        const { data: progressResult, error: progressError } = await supabase
          .from('GamePost')
          .update({ status: 'IN_PROGRESS' })
          .in('id', allFullPosts.map(post => post.id))
          .select('id');
      ...
      // 모이면 바로 출발 글들을 EXPIRED로 변경 (작성 후 6시간 경과)
      const { data: immediateExpiredPosts, error: immediateExpiredError } = await supabase
        .from('GamePost')
        .select('id, title, startTime, status')
        .eq('status', 'OPEN')
        .is('startTime', null)
        .lt('createdAt', sixHoursAgo.toISOString());

      if (immediateExpiredError) {
        throw immediateExpiredError;
      }

      const allExpiredPosts = [...(openPosts || []), ...(immediateExpiredPosts || [])];

      if (allExpiredPosts.length > 0) {
        console.log(`만료할 OPEN 상태 글 ${allExpiredPosts.length}개 발견:`);
        allExpiredPosts.forEach(post => {
          console.log(`- ID: ${post.id}, 제목: ${post.title}, 시작시간: ${post.startTime || '바로출발'}`);
        });
        ...
        const { data: expiredResult, error: expiredError } = await supabase
          ...
          .in('id', allExpiredPosts.map(post => post.id))
          .select('id');
