      ...
      {post.startTime !== undefined && (
        <div className="flex items-center gap-2">
          <div className={`flex items-center gap-2 text-sm font-medium ${post.status === 'COMPLETED' || post.status === 'EXPIRED' ? 'text-muted-foreground' : 'text-primary'}`}>
            ...
            <span className="font-semibold">{isMounted ? (post.startTime ? formatRelativeTime(new Date(post.startTime)) : '모이면 바로 출발') : '...'}</span>
          </div>
        </div>
      )}

      <div className="text-foreground whitespace-pre-wrap break-words leading-relaxed text-base">
        {post.content}
      </div>
    </div>
  );
