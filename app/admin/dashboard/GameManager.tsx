...
export default function GameManager() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 별칭이 비어있으면 빈 배열로, 아니면 쉼표로 분리하여 배열로 변환
      const aliases = formData.aliases.trim() === '' 
        ? [] 
        : formData.aliases
            .split(',')
  ...
  const handleEdit = (game: Game) => {
    ...
    // 별칭이 배열인지 확인하고 적절히 변환
    let aliasesValue = '';
    if (Array.isArray(game.aliases)) {
      aliasesValue = game.aliases.join(', ');
    } else if (game.aliases) {
      aliasesValue = game.aliases;
    }
  ...
  const resetForm = () => {
    setFormData({ name: '', description: '', iconUrl: '', aliases: '' });
    setEditingGame(null);
    setIsAdding(false);
  };
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={handleInputChange}
                  placeholder="쉼표로 구분하여 입력 (예: 롤, 리오레, LOL)"
                  className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                />
