...
이 문서는 G-Club 웹서비스의 일관성 있고 수준 높은 비주얼 아이덴티티를 보장하기 위해 개발자 및 AI 디자인 가이드를 제시합니다. G-Club은 사이버펑크(Cyberpunk) 및 게이밍 감성을 현대적이고 정갈한 **소프트 미니멀리즘(Soft Minimalism)** 스타일로 재해석하여 다크/라이트 테마를 기반으로 디자인되었습니다.

### **소프트 미니멀리즘(Soft Minimalism) 핵심 철학**
1. **글래스모피즘(Glassmorphism) 배제**: 투명 블러(`backdrop-filter: blur()`)나 유리가 겹친 듯한 입체 효과를 배제하고, 깔끔한 단색 채우기와 세련된 단색 경계선을 사용하여 눈의 피로를 최소화하고 콘텐츠 집중도를 높입니다.
2. **플랫 컴포넌트 구조**: 일반 카드, 모달, 드롭다운 등은 물리적이고 깊은 그림자(Shadow) 대신, 은은한 테두리(`border-border`)와 정형화된 백그라운드 색상 대비를 통해서만 구분합니다.
3. **절제된 액센트 네온 글로우**: 게이밍 아이덴티티를 살리기 위해 **핵심 프라이머리 버튼/CTA 요소에 한해서만** 프라이머리 컬러 기반의 은은한 네온 발광(Glow / Box Shadow) 효과를 부분적으로 허용합니다.
...
* **프라이머리 버튼 포인트 네온 글로우 (Neon Glow) 효과**:
  * 일반적인 요소에는 입체감이나 그림자를 사용하지 않는 반면, 브랜드 아이덴티티를 나타내는 주요 프라이머리 버튼(`.btn-primary`)과 주요 CTA 버튼에는 **은은한 프라이머리 컬러 기반의 네온 발광 효과(`box-shadow`)**를 평상시 및 호버 시에 부분적으로 허용합니다.
  * 예시: `box-shadow: 0 0 12px var(--primary-glow);`
* **버튼 호버 트랜지션**:
  * 모든 액티브 요소(버튼, 링크, 탭)는 `transition-all duration-200 ease-in-out`을 기본 장착하여 반응 속도감을 개선합니다.
* **글래스모피즘(Glassmorphism) 배제**:
  * 헤더, 네비게이션 바, 카드 등 어떠한 곳에도 투명 블러 효과(`backdrop-filter: blur()`)를 사용하지 않으며, 단색 불투명 배경을 고수합니다.
